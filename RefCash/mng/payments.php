...
<head>
	...
	<title>Admin Office | Payments | <?php echo $site_name_b; ?></title>
	<style>
		#main>#intro>#img-1 {
	...
	<div id="main">
		...
		<div class="no_of_users">
			<?php
				...
				$py_count_sql = "SELECT COUNT(*) FROM ".$site_name.".`payments`";
				$result = $dbcon->query($py_count_sql);
				$item = mysqli_fetch_array($result, MYSQLI_NUM);
				$number_of_py = $item[0];
				echo $number_of_py." Payments Made";
		...
		<div id="users-div">
			...
			<table id="users-table">
				...
				<?php
					...
					$py_no_of_pages = ceil($number_of_py/10);
					if(isset($_GET['o'])) $off_set = filter_var($_GET['o']); else $off_set = 0;
					if(isset($_GET['rc'])) $row_count = filter_var($_GET['rc']); else $row_count = 10;

					$py_sql = "SELECT `id`, `payed_by`, `email`, `amount`, `benefactor`, `referrence`, `payed_on` FROM ".$site_name_b.".`payments` ORDER BY `payed_on` DESC LIMIT ".$off_set.", ".$row_count;

					$result = $dbcon->query($py_sql);

					while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {

						echo "<tr>";
						echo "<td>".$row['id']."</td>";
						echo "<td>".$row['payed_by']."</td>";
						echo "<td>".$row['email']."</td>";
						echo "<td>".$row['amount']."</td>";
						echo "<td>".$row['benefactor']."</td>";
						echo "<td>".$row['referrence']."</td>";
						echo "<td>".$row['payed_on']."</td>";
						echo "</tr>";
